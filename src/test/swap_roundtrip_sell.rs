...
async fn swap_roundtrip_sell() {
    ...
    let maker_init_response =
        maker_init(maker_addr, qty_from, Some(&asset_id), qty_to, None, 3600, &node2_pubkey).await;
    ...
    assert_eq!(balance_1.offchain_outbound, 10);
    assert_eq!(balance_1.offchain_inbound, 590);
    assert_eq!(balance_2.offchain_outbound, 590);
    assert_eq!(balance_2.offchain_inbound, 10);
    ...
    let chan_2_21 = channels_2
        ...
        .find(|c| c.channel_id == channel_21.channel_id)
        .unwrap();
    // New 2-step swap: HODL payment (taker→maker via chan_21 RGB channel) + forward keysend (maker→taker via chan_12 BTC)
    let hodl_diff_sat = HTLC_MIN_MSAT / 1000; // 3000 sats (carrier msat for RGB HODL invoice)
    let fwd_diff_sat = qty_to / 1000; // 50 sats
    // chan_12: node1→node2 BTC channel, maker sends forward leg
    assert_eq!(
    ...
    // chan_21: node2→node1 RGB channel, taker sends HODL payment
    assert_eq!(
    ...
    wait_for_balance(node1_addr, &asset_id, 10).await;
    wait_for_balance(node2_addr, &asset_id, 990).await;
    ...
    assert_eq!(asset_balance_spendable(node1_addr, &asset_id).await, 5);
    assert_eq!(asset_balance_spendable(node2_addr, &asset_id).await, 290);
    assert_eq!(asset_balance_spendable(node3_addr, &asset_id).await, 705);
}
