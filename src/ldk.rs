...
/// The gossip sync used by the peer manager. Its UTXO lookup is a trait object
/// so it can be backed either by the block-sync `GossipVerifier` or the
/// transaction-sync `IndexerGossipVerifier`.
...
pub(crate) type ChannelManager =
    SimpleArcChannelManager<ChainMonitor, DynBroadcaster, DynFeeEstimator, FilesystemLogger>;
...
pub(crate) async fn start_ldk(
    ...
    // Initialize the chain backend according to the requested sync mode. Both
    // backends provide the FeeEstimator and BroadcasterInterface LDK needs; the
    // transaction-sync backend additionally acts as the chain source (`Filter`)
    // so the ChainMonitor and OutputSweeper only watch relevant transactions.
    ...
    let ChainSetup {
        ...
        LdkChainSync::TransactionSync {
            ...
            // Sync LDK through an indexer. Default to the wallet's `indexer_url`,
            // but allow a dedicated LN indexer so the RGB wallet and LDK can use
            // different sources.
    ...
    // Initialize the ChainMonitor. In transaction-sync mode `chain_filter` is
    // the indexer chain source so only relevant transactions are watched; in
    // block-sync mode it is `None` since full blocks are consumed.
    ...
    // `restarting_node` and `channel_manager_blockhash` are only consumed by the
    // block-sync restart path (`synchronize_listeners`).
    #[cfg_attr(not(feature = "block-sync"), allow(unused_variables))]
    let restarting_node = manager_file.is_ok();
    #[cfg_attr(not(feature = "block-sync"), allow(unused_variables))]
    let (channel_manager_blockhash, channel_manager) = {
        if let Ok(f) = manager_file {
            ...
            (initial_best_block.block_hash, fresh_channel_manager)
        }
    };
    ...
    // Sync ChannelMonitors, ChannelManager and OutputSweeper to the chain tip
    // and hand the monitors to the ChainMonitor. Block-sync replays blocks from
    // bitcoind to bring the listeners up to date before the SPV client takes
    // over; transaction-sync relies on the indexer via the `Confirm` interface,
    // so it just loads the monitors.
    #[cfg(feature = "block-sync")]
    let mut block_sync_cache = UnboundedCache::new();
    // Initialized to `None` for the transaction-sync path (both features on);
    // with only block-sync built it is always set below, hence the allow.
    #[cfg(feature = "block-sync")]
    ...
    // Install a UTXO lookup in the P2PGossipSync so channel announcements can be
    // verified against the funding output. Block-sync fetches full blocks from
    // bitcoind; transaction-sync resolves the funding UTXO through the indexer.
    ...
    // Keep LDK in sync with the chain, using the selected backend.
    let output_sweeper: Arc<OutputSweeper> = Arc::new(output_sweeper);
    let stop_listen = Arc::clone(&stop_processing);
    match backend {
        // Block-sync: poll bitcoind for new blocks and connect/disconnect them.
        #[cfg(feature = "block-sync")]
        ...
        // Transaction-sync: poll the indexer and drive the `Confirm` interface.
        #[cfg(feature = "transaction-sync")]
        ChainBackend::TransactionSync { tx_sync, .. } => {
            ...
            // Bring everything up to the current tip before we start serving.
