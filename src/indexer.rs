...
impl IndexerSyncClient {
    pub(crate) fn new(
        ...
        match protocol {
            ...
            RgbLibIndexerProtocol::Esplora => Ok(Self::Esplora(EsploraSyncClient::new(
                server_url, logger,
            ))),
        }
    }
...
impl BroadcasterInterface for IndexerClient {
    fn broadcast_transactions(&self, txs: &[&Transaction]) {
        match &self.backend {
            IndexerBackend::Electrum(client) => {
                let txs = txs.iter().map(|tx| encode::serialize(*tx)).collect::<Vec<_>>();
        ...
        loop {
            ...
            match res {
                Ok(Ok((background, normal, high_prio, very_high_prio))) => {
                    ...
                    let normal_estimate =
                        fee_rate_from_btc_per_kb(normal, 2000).unwrap_or(2000);
