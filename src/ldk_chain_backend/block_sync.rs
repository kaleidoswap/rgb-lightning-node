...
/// UTXO lookup for gossip verification backed by bitcoind.
///
/// This is the block-sync counterpart to the transaction-sync
/// `IndexerGossipVerifier`. Unlike `lightning-block-sync`'s own
/// `GossipVerifier` (which requires the `P2PGossipSync` to be typed with
/// `Arc<Self>` as its UTXO lookup), this implementation resolves against a
/// `PeerGossipSync` whose lookup is a trait object, so a single `PeerManager`
/// type can serve both sync backends. The funding output for a channel
/// announcement is resolved from full blocks fetched from bitcoind.
...
impl BlockSyncGossipVerifier {
    ...
    async fn retrieve_utxo(
        ...
        'tx_found: {
            ...
            // Serve the funding output from a recently-fetched block when
            // possible so a burst of announcements referencing the same block
            // only fetches it once (ported from lightning-block-sync's own
            // `GossipVerifier`).
            {
